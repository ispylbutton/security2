...
{
  if ((start<<initPosition)&initBit!=0)
  {
  ...
  *nextKey=0;
  ...
  if (roundNum==0)
  {
    for (i=0; i<56; i++)
    {
      if (i<28)
      {
	bitwiseAddition(* initKey, &leftKey, PC[i]-1, i);
      }
      else
      {
  ...
  tempLeftKey+=(leftKey<<shiftBy[round];
  tempRightKey+=(rightKey<<shiftBy[round]);
...
void singleRoundDES(uint64_t * leftBlock, uint64_t key)
{
  ...
  for (i=0; i<8; i++)
  {
    ...
    subBlock=SBox[i][x][y];///change here
